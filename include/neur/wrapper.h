...
template<class T>
class dynamic_wrapper:public wrapper<T>{
...
template<class T>
auto dynamic_wrapper<T>::insert(const iterator it, const value_type &l) -> iterator{
    return m_data.insert(it, l);
}
template<class T>
auto dynamic_wrapper<T>::insert(const iterator it, value_type &&l) -> iterator{
    return m_data.insert(it, std::move(l));
}
template<class T>
auto dynamic_wrapper<T>::erase(const iterator &it) -> iterator{
    return m_data.erase(it);
}
template<class T>
auto dynamic_wrapper<T>::erase(iterator &&it) -> iterator{
    return m_data.erase(it);
}
template<class T>
void dynamic_wrapper<T>::reserve(const size_t &s){
    m_data.reserve(s);
}
template<class T>
void dynamic_wrapper<T>::resize(const size_t &s){
    m_data.resize(s);
}
template<class T>
auto dynamic_wrapper<T>::clear(){
    m_data.clear();
}
};